...
function detectCardType(cardNumber) {
  var patterns = {
    visa: /^4\d{12}(\d{3})?(\d{3})?$/,
    master: /^(5[1-5]\d{14}|2(2[2-9]\d{2}|[3-6]\d{3}|7[01]\d{2}|720\d{2})\d{10})$/,
    amex: /^3[47]\d{13}$/,
    mir: /^220[0-4]/,
    discover: /^(6011|65\d{2}|64[4-9]\d|622(1[2-9]|[2-8]\d|9[01])\d{3})\d{10,16}$/,
...
function disabledImage(arrImage, type) {
  var arr = Array.from(arrImage);
  arr.filter(function (image) {
...
var inputNumberCard = document.querySelector(".input_card_number");
var imageCards = document.querySelectorAll(".card");
var btn = document.querySelector(".button_valid");
function main() {
  ...
  _main = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          cardNumber = inputNumberCard;
          btn.addEventListener("click", function () {
            activeImage(imageCards);
            if (validateLuhn(cardNumber.value)) {
              var type = detectCardType(cardNumber.value);
